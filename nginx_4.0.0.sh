...
# scp nginx_4.0.0.sh root@server_ip:~/
# Grant permissions to the script:
# sudo chmod +x nginx_4.0.0.sh
# Execute the file:
# sudo ./nginx_4.0.0.sh
...
SCRIPT_NAME="nginx_4.0.0.sh"

# version 4.0.0 [2022-05-23]
# added cloudflare certification
# added IP domain and server check
# added distinction of SSL snippets for Cloudflare and Let's Encrypt
# added IP access blockade
# added loop of certifications
# added Proxy_mode switch to True
...
if [[ "$NGINX_OUTPUT" == *"syntax is ok"*"successful" ]]; then
	...
	sed -i 's+proxy_mode = False+proxy_mode = True+' /etc/odootest-server.conf
	sed -i 's+proxy_mode = False+proxy_mode = True+' /etc/odooprod-server.conf
	sudo systemctl restart nginx
	sudo systemctl restart odooprod-server.service
	sudo systemctl restart odootest-server.service
	sleep 1
